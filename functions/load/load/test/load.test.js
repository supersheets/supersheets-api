...
describe('Error Handling', () => {
  ...
  it ('should throw error fetching a sheet from Google Sheets API', async () => {
    ...
    ctx.state.sheets = mockaxios(null, new Error("Error fetching from Google Sheets"))
    ctx.state.docs = mockaxios({ hello: "world" })
    await func.invoke(ctx)
    expect(ctx.error).toBeTruthy()
...
describe('Load', () => {
  ...
  it ('should do a reload with no datatypes config', async () => {
    ...
    let status = await statuslib.getStatus(db, { uuid: "STATUS-UUID" })
